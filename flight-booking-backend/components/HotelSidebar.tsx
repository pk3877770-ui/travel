    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
        <div className="relative h-[200px] w-full bg-slate-100">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <button className="bg-slate-900 text-white flex items-center gap-2 px-4 py-2 rounded font-medium text-sm hover:bg-slate-800 transition-colors shadow-lg">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button className="text-sm font-bold text-blue-600 hover:text-blue-800">Clear All</button>
        </div>

        {/* Price Slider */}
        <div className="mb-8">
          <h4 className="font-bold text-sm text-slate-800 mb-3">Price per night</h4>
          <div className="text-xs text-slate-500 mb-2">₹1,000 - ₹20,000+</div>
          <div className="relative h-1 bg-slate-200 rounded-full mt-4">
            ...
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-4 h-4 bg-blue-600 rounded-full shadow border-2 border-white"></div>
            <div className="absolute right-1/4 top-1/2 -translate-y-1/2 w-4 h-4 bg-blue-600 rounded-full shadow border-2 border-white"></div>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {[
              { label: "Luxury (5 Stars)", count: 120 },
              { label: "Upscale (4 Stars)", count: 95 },
              { label: "Mid-range (3 Stars)", count: 70 },
              { label: "Budget (1-2 Stars)", count: 27 },
            ].map((item, idx) => (
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {[
              { label: "4.5 & above", count: 85 },
              { label: "4.0 & above", count: 140 },
              { label: "3.5 & above", count: 210 },
              { label: "3.0 & above", count: 260 },
            ].map((item, idx) => (
              <label key={idx} className="flex items-center justify-between cursor-pointer group">
                <div className="flex items-center gap-3">
                  <div className="w-4 h-4 border border-slate-300 rounded flex items-center justify-center group-hover:border-blue-500 transition-colors" />
        ...
        <div>
          ...
          <div className="space-y-3">
            {[
              { label: "Free Wi-Fi", count: 290 },
              { label: "Breakfast included", count: 200 },
              { label: "Pool", count: 120 },
              { label: "Free Cancellation", count: 180 },
              { label: "Airport Shuttle", count: 90 },
            ].map((item, idx) => (
              <label key={idx} className="flex items-center justify-between cursor-pointer group">
                <div className="flex items-center gap-3">
                  <div className="w-4 h-4 border border-slate-300 rounded flex items-center justify-center group-hover:border-blue-500 transition-colors" />
